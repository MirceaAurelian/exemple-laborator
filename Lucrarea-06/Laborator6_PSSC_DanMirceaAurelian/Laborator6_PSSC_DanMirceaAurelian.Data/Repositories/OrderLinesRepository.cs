...
namespace Laborator6_PSSC_DanMirceaAurelian.Data.Repositories
{
    public class OrderLinesRepository : IOrderLinesRepository
    {
        ...
        {
            var orders = (await dbContext.OrderHeaders.AsNoTracking().ToListAsync()).ToLookup(oh => oh.OrderId);
            var order_lines = (await dbContext.OrderLines.AsNoTracking().ToListAsync()).ToLookup(ol => ol.ProductId);
            ...
            var updatedProductStocs = products.Select(p => products[p.Key].Single()).Select(p => new ProductDto()
                                                                                    {
                                                                                        ...
                                                                                        Code = p.Code,
                                                                                        Stoc = p.Stoc + order_lines[p.ProductId].Select(ol => ol.Quantity).Sum() - shoppingCarts.ShoppingCartList.Where(sp => sp.productCode.Code == p.Code).Select(sp => sp.quantity.Value).Sum()
                                                                                    });
