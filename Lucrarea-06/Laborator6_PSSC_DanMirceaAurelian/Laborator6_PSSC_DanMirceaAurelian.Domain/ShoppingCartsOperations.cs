...
namespace Laborator6_PSSC_DanMirceaAurelian.Domain
{
    ...
    {
        public static Task<IShoppingCarts> ValidateShoppingCarts(Func<ProductCode, Option<ProductCode>> checkProductExists, Func<int, Option<int>> checkOrderHeaderExistsOrInsert, Func<ProductCode, Option<ProductCode>> checkProductStocValid, EmptyShoppingCarts shoppingCarts) =>
            shoppingCarts.ShoppingCartList
                      .Select(ValidateShoppingCart(checkProductExists, checkOrderHeaderExistsOrInsert, checkProductStocValid))
        ...
        private static Func<EmptyShoppingCart, EitherAsync<string, ValidatedShoppingCart>> ValidateShoppingCart(Func<ProductCode, Option<ProductCode>> checkProductExists, Func<int, Option<int>> checkOrderHeaderExistsOrInsert, Func<ProductCode, Option<ProductCode>> checkProductStocValid) =>
        emptyShoppingCart => ValidateShoppingCart(checkProductExists, checkOrderHeaderExistsOrInsert, checkProductStocValid, emptyShoppingCart);

        private static EitherAsync<string, ValidatedShoppingCart> ValidateShoppingCart(Func<ProductCode, Option<ProductCode>> checkProductExists, Func<int, Option<int>> checkOrderHeaderExistsOrInsert, Func<ProductCode, Option<ProductCode>> checkProductStocValid, EmptyShoppingCart emptyShoppingCart) =>
            ...
            from checkproductStocValid in checkProductStocValid(productCode)
                        .ToEitherAsync(() => $"Not enough stock for product {productCode}.")
            from productExists in checkProductExists(productCode)
                   .ToEitherAsync($"Product {productCode} does not exist.")
            from checkorderHeaderExistsOrInsert in checkOrderHeaderExistsOrInsert(emptyShoppingCart.OrderId)
        ...
        {
            /*   Cases to treat: */
            /*   New order header (OrderId == 0 && IsUpdated == 0) */
            /*   Existing order header (OrderId > 0 && IsUpdated == 2), new product to add (insert), update needed for address, finalprice (OrderHeader), stoc (Product) */
            /*   Existing order header (OrderId > 0 && IsUpdated == 1), existing order product */
